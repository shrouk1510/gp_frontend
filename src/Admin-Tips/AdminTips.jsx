...
const AdminTips = () => {
  const [pages, setPages] = useState([
    { 
      ...
      content: `You may have heard that diabetes can be controlled by diet. There is no such thing as a special diet exclusively for people with diabetes. There are a lot of different ways to lose weight – but there’s no one-size-fits-all diet. It starts with finding a way to eat fewer calories than you need. A calorie (or kcal) is a unit of energy, which is in the food and drink we consume. Your body uses energy for everything we do – from breathing and sleeping to exercising. When you eat, you’re replacing the energy you’ve used, which helps you to maintain a healthy weight. As a general guide, government recommendations are that men need around 2,500kcal a day to maintain a healthy weight, and women need around 2,000kcal a day. But most people need different amounts of calories based on how their bodies work, how active they are and any weight management goals.`, 
      editing: false 
    }, 
    { 
      id: 2, 
  ...
  const handleEdit = (id, content) => {
    setPages(
      pages.map(page =>
        page.id === id ? { ...page, editing: true } : { ...page, editing: false }
      )
    );
    setNewContent(content); // Set current content in the form
  };

  const handleSave = (id) => {
    setPages(
      pages.map(page =>
        page.id === id ? { ...page, content: newContent, editing: false } : page
      )
    );
  };
        ...
        <div key={page.id} className="page">
          ...
          {page.id !== 5 && page.id !==7 && !page.editing ? (
            <div>
              ...
              <div className="bb">
                <button className="edit" onClick={() => handleEdit(page.id, page.content)}>Edit</button>
            ...
            <div>
              ...
              />
              <div className="bb">
                <button className="save" onClick={() => handleSave(page.id)}>Save</button>
              </div>
            </div>
          ) : page.id===5 ? (
            <LifeStyleAdmin />
